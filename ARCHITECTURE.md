...
validated canonical form rather than detecting specific exploits. That is the only defense that
survives a Mythos-class, infinite-bug-discovery adversary — you don't need to know the CVE.
...
def check_length_amplification(filepath) -> [Finding]  # NEW — headline parser-DoS defense
    ...
    # Flag declared_length >> remaining (ratio threshold) — the 140B→4GB / GDCM CVE-2026-3650 class.
    # Logic already prototyped in _attack_test/prove_attack_surface.py (demo 3).

def check_sequence_depth(ds) -> [Finding]            # NEW
    # Recursively walk SQ elements; flag nesting depth > ~10 (normal 3–5). Nesting bombs exhaust memory.
...
def check_crypto_posture(ds, context=None) -> [Finding]
    ...
    #   alone — report what's knowable + prompt for transport context if integrated inline.
